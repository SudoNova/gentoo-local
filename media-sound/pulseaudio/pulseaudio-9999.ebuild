...
EAPI=6
inherit autotools bash-completion-r1 flag-o-matic git-r3 gnome2-utils linux-info systemd user udev multilib-minimal
...
src_unpack() {
	# Autoconf gets version string from `git describe` so tags are necessary.
...
src_prepare() {
	...
	[[ ! -f src/Makefile.am ]] || sed -i -e '/lock-autospawn-test /d' src/Makefile.am || die
	[[ ! -f src/Makefile.in ]] || sed -i -e '/lock-autospawn-test $(EXEEXT) //' src/Makefile.in || die
